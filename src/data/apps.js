export const apps = [
  {
    id: 'bday-recap',
    emoji: '🎂',
    name: '성호 생일 축하 페이지',
    description: '사진드컵·유형테스트·실시간 롤링페이퍼. 실사용자가 있는 팬 프로젝트',
    tags: ['Next.js', 'Supabase', 'Realtime', 'AI'],
    difficulty: '⭐⭐⭐',
    external: 'https://bday-recap.vercel.app',
    color: '#FFEFF5',
    highlight: true,
    devDiary: '',
  },
  {
    id: 'kings-warden',
    emoji: '👑',
    name: '왕과 사는 남자 유형 테스트',
    description: '10문항으로 알아보는 나의 캐릭터 유형. 역사 드라마 테마 성격 테스트',
    tags: ['Next.js', '유형테스트', 'SEO'],
    difficulty: '⭐⭐',
    external: 'https://the-kings-warden2.vercel.app/',
    color: '#E8EAF6',
    devDiary: '',
  },
  {
    id: 'bnd-recap',
    emoji: '🎊',
    name: 'BOYNEXTDOOR 2025 RECAP',
    description: '한 해의 순간을 모은 팬 리캡 페이지. 공유용 카드 미리보기 지원',
    tags: ['팬페이지', 'Recap', 'OG'],
    difficulty: '⭐⭐',
    external: 'https://bnd-recap-project.netlify.app/',
    color: '#FFF3E0',
    devDiary: '',
  },
  {
    id: 'pic-bti',
    emoji: '📸',
    name: '픽BTI',
    description: '사진 한 장 올리면 AI가 오늘의 MBTI를 분석하고 30초 팟캐스트로 수다떨어요',
    tags: ['Kanana-o', '멀티모달 AI', 'TTS'],
    difficulty: '⭐⭐⭐',
    path: '/pic-bti',
    showcase: true, // 실전 프로젝트 섹션에 표시 (상세 소개 페이지로 연결)
    color: '#E0F7FA',
    highlight: true,
    devDiary: '',
  },
  {
    id: 'reading-log',
    emoji: '📚',
    name: '독서 기록',
    description: '읽은 책을 기록하고 별점과 감상을 남겨요',
    tags: ['localStorage', 'CRUD'],
    difficulty: '⭐',
    path: '/reading-log',
    color: '#FFE4EC',
    devDiary: 'https://www.notion.so/333bb3574f3081f29974e4d7d968c227',
  },
  {
    id: 'movie-log',
    emoji: '🎬',
    name: '영화/드라마 기록',
    description: '보고싶은 작품을 기록하고 별점과 감상을 남겨요',
    tags: ['엔터테인먼트', 'TMDB', '기록'],
    difficulty: '⭐⭐',
    path: '/movie-log',
    color: '#FFE4EC',
    devDiary: 'https://www.notion.so/33ebb3574f3081b18eaadf0d6329398e',
  },
  {
    id: 'pomodoro',
    emoji: '⏱️',
    name: '포모도로 타이머',
    description: '25분 집중, 5분 휴식. 생산성을 높여요',
    tags: ['타이머', '기록'],
    difficulty: '⭐',
    path: '/pomodoro',
    color: '#E8F5E9',
    devDiary: 'https://www.notion.so/333bb3574f308113ac67e9ae38f29971',
  },
  {
    id: 'retro-board',
    emoji: '📋',
    name: '태스크 보드',
    description: '할 일을 카드로 관리하고 드래그로 이동해요',
    tags: ['칸반', '드래그앤드롭'],
    difficulty: '⭐⭐',
    path: '/retro-board',
    color: '#E3F2FD',
    devDiary: 'https://www.notion.so/4c5905c527e24951b55b1bc360576888',
  },
  {
    id: 'habit-tracker',
    emoji: '✅',
    name: '습관 트래커',
    description: '매일 체크하고 연속 달성 스트릭을 쌓아요',
    tags: ['히트맵', '스트릭'],
    difficulty: '⭐⭐',
    path: '/habit-tracker',
    color: '#F3E5F5',
    devDiary: 'https://www.notion.so/333bb3574f30818fb08fc4d51975f1b',
  },
  {
    id: 'budget',
    emoji: '💰',
    name: '간단한 가계부',
    description: '수입과 지출을 기록하고 차트로 확인해요',
    tags: ['Chart.js', '통계'],
    difficulty: '⭐⭐',
    path: '/budget',
    color: '#E0F7FA',
    devDiary: 'https://www.notion.so/334bb3574f30810f91b5f320cb4c3bea',
  },
  {
    id: 'ai-diary',
    emoji: '🤖',
    name: 'AI 일기 도우미',
    description: '일기를 쓰면 AI가 감정을 분석하고 위로해줘요',
    tags: ['Claude API', 'AI'],
    difficulty: '⭐⭐⭐',
    path: '/ai-diary',
    color: '#FFF8E1',
    highlight: true,
    devDiary: '',
  },
  {
    id: 'mood-tracker',
    emoji: '😌',
    name: '무드 트래커',
    description: '오늘 기분을 기록하고 Claude의 한 마디를 받아요',
    tags: ['감정', 'AI', '기록'],
    difficulty: '⭐⭐',
    path: '/mood-tracker',
    color: '#FFE4EC',
    devDiary: '',
  },
  {
    id: 'dashboard',
    emoji: '📊',
    name: '통합 대시보드',
    description: '모든 앱의 데이터를 한눈에 요약해서 봐요',
    tags: ['요약', '통계'],
    difficulty: '⭐',
    path: '/dashboard',
    color: '#E3F2FD',
    devDiary: 'https://www.notion.so/33ebb3574f3081898df8cf4d51975f1b',
  },
]
